 ...
 */
function shouldEnableSwipeGesture() {
  if (_reactNative.Platform.OS === 'web' && typeof navigator !== 'undefined' && typeof window !== 'undefined') {
    const isWebAppleDevice = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
...
class DrawerView extends React.Component {
  constructor(...args) {
    ...
    })), onChange(this.gestureState, cond(eq(this.gestureState, _reactNativeGestureHandler.State.ACTIVE), call([], this.handleStartInteraction))), cond(eq(this.gestureState, _reactNativeGestureHandler.State.ACTIVE), [cond(this.isSwiping, NOOP, [// We weren't dragging before, set it to true
  ...
  render() {
    ...
    const drawerTranslateX = drawerType === 'back' ? _reactNative.I18nManager.isRTL ? multiply(sub(this.containerWidth, this.drawerWidth), isRight ? 1 : -1) : 0 : this.translateX;
    ...
    return /*#__PURE__*/React.createElement(_reactNativeGestureHandler.PanGestureHandler, _extends({
      ref: onGestureRef,
    ...
    drawerType === 'permanent' ? null : /*#__PURE__*/React.createElement(_reactNativeGestureHandler.TapGestureHandler, {
      ...
      style: [styles.container, drawerType === 'permanent' ? // Without this, the `left`/`right` values don't get reset
      isRight ? {
        right: 0
      } : {
        left: 0
